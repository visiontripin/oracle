# Промт: генерация ботов для BotControl (v5.2, под импортёр v1.6.2)
...
**Новое в v5:** «фишки» — анимация правкой одного сообщения
(`play_animation`: заставки, прогресс-бар, спиннер, флипбук, текстовый
квест, слот-машина) и кубики Telegram (`bot.send_dice`). Правила 10–11.
v5.1: кадры-картинки `photo: ссылка` (editMessageMedia).
v5.2: слайды кнопками — кнопка с фото-кадром и `message_id` меняет картинку
в том же сообщении, меню остаётся; до 12 кнопок под сообщением.
